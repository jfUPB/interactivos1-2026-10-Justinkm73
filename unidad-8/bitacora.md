...
### ITERACIÓN INGENIERIL // Actividad #1

### Diagrama de la arquitectura

<img width="1536" height="1024" alt="Img" src="https://github.com/user-attachments/assets/93e69922-4262-48d8-a39d-6fed789c473e" />


--


El sistema integra las tres fuentes de datos requeridas a través de adaptadores independientes que respetan estrictamente el flujo:

```Adapter → bridgeServer → bridgeClient → FSMTask → updateLogic → drawRunning```

Cada fuente entra por su propio adaptador, normaliza sus datos a un contrato claro y los entrega a bridgeServer.js, que se encarga únicamente del transporte y la distribución de mensajes.
...
**A.) MicrobitBinaryAdapter**

Recibe datos del micro:bit a través del puerto serial en protocolo binario. Cada paquete tiene un tamaño fijo de 8 bytes con la siguiente estructura:

```[0xAA] [X_high] [X_low] [Y_high] [Y_low] [btnA] [btnB] [checksum]```

El adaptador acumula los bytes entrantes en un buffer, busca el byte de sincronización 0xAA, valida el checksum (suma de bytes 1 a 6, módulo 256) y parsea los valores. Si el checksum no coincide, el paquete se descarta y se avanza un byte para no perder el siguiente paquete válido.

Contrato de salida:

```{ "x": -312, "y": 104, "btnA": false, "btnB": true }```
...
Levanta un servidor WebSocket en el puerto 8080. Strudel se conecta a este servidor y envía sus eventos musicales como mensajes OSC-over-WebSocket con dirección ```/dirt/play```.

El adaptador parsea el array plano de argumentos ```([clave, valor, clave, valor, ...])``` convirtiéndolo en un objeto, clasifica el tipo de sonido a partir del campo ```s``` (```"bd"```, ```"sd"```, ```"hh"```, ```"other"```) y entrega un objeto normalizado.
...
**C.) OpenStageControlAdapter**

Escucha mensajes OSC por UDP en el puerto 9000. Cada vez que el usuario mueve un control en Open Stage Control, llega un mensaje con una dirección y sus argumentos. El adaptador aplana los argumentos (que la librería osc puede entregar con o sin metadatos de tipo) y entrega el objeto normalizado.
